...
description: "Frequently asked questions and solutions to common issues with Gauss Agent"
---
...
### What LLM providers work with Gauss?

Gauss Agent works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `gauss model` or by editing `~/.gauss/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Gauss Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Gauss from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/NousResearch/gauss-agent/main/scripts/install.sh | bash
```

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Gauss Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.gauss/`.

### Can I use it offline / with local models?

Yes. Point Gauss at any local OpenAI-compatible server:

```bash
gauss config set OPENAI_BASE_URL http://localhost:11434/v1  # Ollama
gauss config set OPENAI_API_KEY ollama                       # Any non-empty value
gauss config set GAUSS_MODEL llama3.1
```

You can also save the endpoint interactively with `gauss model`. Gauss persists that custom endpoint in `config.yaml`, and auxiliary tasks configured with provider `main` follow the same saved endpoint.
...
Gauss Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Gauss Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Gauss programmatically:

```python
from gauss.agent import AIAgent

agent = AIAgent(model="openrouter/nous/gauss-3-llama-3.1-70b")
...
#### `gauss: command not found` after installation
...
**Cause:** Gauss requires Python 3.11 or newer.
...
gauss config get OPENROUTER_API_KEY

# Re-configure your provider
gauss model

# Or set directly
gauss config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```
...
# Set a valid model
gauss config set GAUSS_MODEL openrouter/nous/gauss-3-llama-3.1-70b

# Or specify per-session
gauss chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `gauss chat --provider <alternative>` to route to a different backend
...
gauss chat --model openrouter/google/gemini-2.0-flash-001
```
...
**Cause:** Gauss detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Gauss never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `gauss chat`
...
**Solution:**
- Verify your bot token is valid with `gauss gateway setup`
- Check gateway logs: `gauss gateway logs`
...
Configure in `~/.gauss/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install gauss-agent[telegram]   # or [discord], [slack], [whatsapp]
...
**Solution:**
- Try a faster/smaller model: `gauss chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `gauss chat -t "terminal"`
...
Verify your `~/.gauss/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
gauss config show | grep -A 12 mcp_servers

# Restart Gauss or reload MCP after config changes
gauss chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Gauss](/docs/guides/use-mcp-with-gauss)
...
:::warning
If an MCP server crashes mid-request, Gauss will report a timeout. Check the server's own logs (not just Gauss logs) to diagnose the root cause.
:::
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/gauss-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Gauss version (`gauss --version`), and the full error message
